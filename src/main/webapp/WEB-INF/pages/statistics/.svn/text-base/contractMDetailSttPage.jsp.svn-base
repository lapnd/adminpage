		...
		<div class="content">
			...
			<thead>
				<tr>
					...
					<th id="areaSend">Number of message</th>
				</tr>
			</thead>
			<tbody>
			<c:if test="${fn:length(lstItemStt) >0 }">
				<c:forEach items="${lstItemStt}" varStatus="statusMessageStt" var="itMessage">
					<tr>
						<td>
							<fmt:formatDate pattern="${varMonthStt=='-1'?'MM, yyyy':'MM/dd, yyyy'}" 
											value="${itMessage.date}" />
						</td>
						<td>${itMessage.receivedNo}</td>
					</tr>
				</c:forEach>
			</c:if>
			</tbody>
